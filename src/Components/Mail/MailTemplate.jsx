...
const MailForm = () => {
 ...
 const navigate = useNavigate()
  ...
  const handleSubmit = async (e) => {
    ...
    formData.BasicWebMailIntro && (payload.BasicWebMailIntro = formData.BasicWebMailIntro);
    formData.BasicWebMailMainBody && (payload.BasicWebMailMainBody = formData.BasicWebMailMainBody);
    formData.BasicWebMailList && (payload.BasicWebMailList = formData.BasicWebMailList);
    formData.BasicWebMailConclude && (payload.BasicWebMailConclude = formData.BasicWebMailConclude);
    formData.BasicWebMailLink?.length && (payload.BasicWebMailLink = formData.BasicWebMailLink);
    
    formData.DmMailIntro && (payload.DmMailIntro = formData.DmMailIntro);
    formData.DmMailMainBody && (payload.DmMailMainBody = formData.DmMailMainBody);
    formData.DmMailList && (payload.DmMailList = formData.DmMailList);
    formData.DmMailConclude && (payload.DmMailConclude = formData.DmMailConclude);
    ...
    formData.EcomMailMainBody && (payload.EcomMailMainBody = formData.EcomMailMainBody);
    formData.EcmoMailList && (payload.EcmoMailList = formData.EcmoMailList);
    formData.EcomMailConclude && (payload.EcomMailConclude = formData.EcomMailConclude);
    formData.EcomMailLink?.length && (payload.EcomMailLink = formData.EcomMailLink);
  
    formData.SeoMailIntro && (payload.SeoMailIntro = formData.SeoMailIntro);
    formData.SeoMailMainBody && (payload.SeoMailMainBody = formData.SeoMailMainBody);
    formData.SeoMailList && (payload.SeoMailList = formData.SeoMailList);
    formData.SeoMailConclude && (payload.SeoMailConclude = formData.SeoMailConclude);
    formData.SeoMailLink?.length && (payload.SeoMailLink = formData.SeoMailLink);
  
    formData.SmoMailIntro && (payload.SmoMailIntro = formData.SmoMailIntro);
    formData.SmoMailMainBody && (payload.SmoMailMainBody = formData.SmoMailMainBody);
    formData.SmoMailList && (payload.SmoMailList = formData.SmoMailList);
    formData.SmoMailConclude && (payload.SmoMailConclude = formData.SmoMailConclude);
    formData.SmoMailLink?.length && (payload.SmoMailLink = formData.SmoMailLink);
  
    formData.user_id && (payload.user_id = formData.user_id);
  
    try {
      const response = await axios.post(`${import.meta.env.VITE_BACKEND_API}/mailData`, payload);
        ...
        <Form.Item label="Basic Web Mail Links">
          {formData.BasicWebMailLink.map((link, index) => (
            <Input.TextArea
              key={index}
              value={link}
              onChange={(e) => handleArrayChange(e, 'BasicWebMailLink', index)}
